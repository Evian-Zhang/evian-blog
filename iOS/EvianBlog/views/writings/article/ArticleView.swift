    ...
    var body: some View {
		List {
			ForEach(self.viewModel.articles.enumerated().map({ $0 }), id: \.1.title) { (index, articleMeta) in
				ArticleRowView(articleRowViewModel: ArticleRowViewModel(articleMeta: articleMeta, dateFormatter: self.dateFormatter, writingsViewDelegate: self.viewModel.writingsViewDelegate))
					.onAppear(perform: {
...
struct ArticleView_Previews: PreviewProvider {
	...
	private static let articleViewModel = ArticleViewModel(blogAPI: blogAPI, writingsViewDelegate: WritingsViewModel(blogAPI: blogAPI))
